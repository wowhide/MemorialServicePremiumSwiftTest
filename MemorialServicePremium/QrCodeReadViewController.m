...
//            NSLog(@"%@", qrcode);
    ...
    if (qrReadFlg == YES) {
        ...
        if (self.fromView == QR_READ_VIEW_FROM_INITIAL_SETTING) {
            alertTitle = [NSString stringWithFormat:@"%@様のデータを読み込みます。\nよろしいですか？", deceasedName];
            alertMessage = @"";
        } else if (self.fromView == QR_READ_VIEW_FROM_DECEASED_LIST) {
            alertTitle = [NSString stringWithFormat:@"%@様のデータを追加します。\nよろしいですか？", deceasedName];
...
- (void)downloadDeceasedData:(NSString *)deceasedId
{
    ...
    @try {
        ...
        if (deceasedJsonData && readdeceaseddata_response.statusCode == 200) {
            ...
            //ポイント機能利用者IDが登録されている場合のみ処理する
            if ([userid length]) {
                // URL指定、パラメーターを付与
                NSString *urlAsString = POINT_ADD_USER1;
