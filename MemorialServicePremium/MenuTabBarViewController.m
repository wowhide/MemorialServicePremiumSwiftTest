...
- (void)viewDidLoad
{
    ...
    _otherNavViewController.tabBarItem.title = @"その他";
    _otherNavViewController.tabBarItem.image = [UIImage imageNamed:@"tab_other"];

    NSArray *views = [NSArray arrayWithObjects:_deceasedListNavViewController, _noticeSettingViewController, _otherNavViewController, nil];
...
{
    //利用規約画面を表示する
    MailInputViewController *mailInputViewController = [MailInputViewController new];
    mailInputViewController.delegate = self;
    [self addChildViewController:mailInputViewController];
    [self.view addSubview:mailInputViewController.view];
    [mailInputViewController didMoveToParentViewController:self];
}
